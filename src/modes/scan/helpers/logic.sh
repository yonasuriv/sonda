function system_info {
    ...
    hostname=$(read_system_file "/etc/hostname")

    # Fetch static hostname from /proc/sys/kernel/hostname
    static_hostname=$(read_system_file "/proc/sys/kernel/hostname")
    ...
    kernel_name=$(uname -s)
    kernel_version=$(read_system_file "/proc/version" | awk '{print $3}' || echo "Unknown")
    kernel_release=$(read_system_file "/proc/sys/kernel/osrelease")
    kernel_version_full=$(read_system_file "/proc/sys/kernel/version")
    ...
    firmware_date=$(read_system_file "/sys/class/dmi/id/bios_date")

    # Get the current date in seconds since the epoch
    current_date_seconds=$(date +%s)

    # Get the firmware date in seconds since the epoch
    if [[ "$firmware_date" != "Unknown" ]] && [[ -n "$firmware_date" ]]; then
        firmware_date_seconds=$(date -d "$firmware_date" +%s 2>/dev/null || echo "0")
        # Calculate the difference in seconds
        if [[ "$firmware_date_seconds" != "0" ]] && [[ "$firmware_date_seconds" =~ ^[0-9]+$ ]]; then
            seconds_diff=$((current_date_seconds - firmware_date_seconds))
        else
            seconds_diff=0
        fi
    else
        firmware_date_seconds="0"
        seconds_diff=0
    fi
    ...
    # Fetch machine ID
    machine_id=$(read_system_file "/etc/machine-id")

    # Fetch boot ID
    boot_id=$(read_system_file "/proc/sys/kernel/random/boot_id")

    stats_cpu_mem_disk() {
        local line1=""  # Initialize line1 to avoid unbound variable error with set -u
        ...
        echo -e " |  CPU  $(mpstat | awk 'NR==4 {print "User: "$3 "%, System: "$5 "%, Idle: "$12 "%"}')${RT}" 
        echo -e " |  MEM  $(free -h | awk 'NR==2 {print "Total: "$2 ", Used: "$3 ", Free: "$4 ", Available: "$7}')${RT}"
        df -h | grep '^/dev/' | while read -r line; do
            ...
            device_name=$(basename "$device")
                ...
                if [ "$usage" -ge 75 ]; then
                    output="${RED}[$device_name]${RT} at ${RED}$usage%${RT} ($total total, $used used, $available available)${RT}"
                elif [ "$usage" -ge 50 ]; then
                    output="${YELLOW}[$device_name]${RT} at ${YELLOW}$usage%${RT} ($total total, $used used, $available available)${RT}"
                else
    ...
    # Purpose: Extract recently installed package names from /var/log/apt/history.log safely.
    # Notes: Outputs packages space-separated, single spaces, no commas.

    lastipkgs() {
    local log="/var/log/apt/history.log"
    local want="${1:-10}"

    [[ -r "$log" ]] || { echo "missing: $log" >&2; return 1; }

    awk -v want="$want" '
        function is_opt(t) { return (t ~ /^-/) }
        function is_deb(t) { return (t ~ /\.deb$/) }

        /^Commandline:/ {
        line = substr($0, index($0,$2))
        gsub(/^[ \t]+/, "", line)

        if (line !~ /(^|[ \t])(apt|apt-get)[ \t]+install([ \t]|$)/) next

        n = split(line, a, /[ \t]+/)
        for (i = 1; i <= n; i++) {
            if (a[i] == "apt" || a[i] == "apt-get") continue
    ...
    # Get network status with VPN/protected/exposed detection
    get_network_status() {
        # First check basic connectivity
        if ! ping -c 1 -W 2 1.1.1.1 >/dev/null 2>&1; then
            echo -e "${NKBRRED}[NO INTERNET]${RT}"
            return
        fi
        
        # Try to get public IP info using curl (faster and more reliable than Python script for simple status)
        if ! command -v curl >/dev/null 2>&1; then
            # Fallback to simple ping check
            echo -e "${NKBRGREEN}[NETWORK CONNECTED]${RT}"
            return
        fi
        ...
        # Extract ISP and hostname
        isp=$(echo "$ip_info" | grep -o '"org":"[^"]*' | cut -d'"' -f4 || echo "")
        hostname=$(echo "$ip_info" | grep -o '"hostname":"[^"]*' | cut -d'"' -f4 || echo "")
        
        # VPN/Hosting/Cloud indicators (matching Python script logic)
        vpn_indicators="VPN VPS Hosting Colo Datacenter DC Server Amazon AWS Azure GCP Google Microsoft Tailscale DigitalOcean Linode OVH Hetzner Contabo Oracle Netcup UpCloud Vultr Scaleway Kamatera IBMCloud Fortinet Cisco Juniper Palo Alto GlobalProtect AnyConnect SonicWall Checkpoint Citrix NetScaler F5 BIG-IP NordVPN ExpressVPN Surfshark Proton IPVanish TunnelBear PIA PrivateInternetAccess Windscribe CyberGhost Hotspot Datacamp Datapacket"
        
        # Check if ISP or hostname contains VPN indicators (case-insensitive, minimum 4 chars)
        is_vpn=0
        isp_lower=$(echo "$isp" | tr '[:upper:]' '[:lower:]')
        hostname_lower=$(echo "$hostname" | tr '[:upper:]' '[:lower:]')
        
        for indicator in $vpn_indicators; do
            if [[ ${#indicator} -ge 4 ]]; then
                indicator_lower=$(echo "$indicator" | tr '[:upper:]' '[:lower:]')
                if [[ "$isp_lower" == *"$indicator_lower"* ]] || [[ "$hostname_lower" == *"$indicator_lower"* ]]; then
                    is_vpn=1
                    break
                fi
            fi
        ...
        if [[ "$is_vpn" -eq 1 ]]; then
            echo -e "${NKBRGREEN}[VPN PROTECTED]${RT}"
        else
            echo -e "${NKBRYELLOW}[PUBLIC IP EXPOSED]${RT}"
        fi
    }
    
    calculate_time_since_last_update() {
        # Initialize LAST_UPDATE_DATE if not set
    ...
    firmware_vendor="$(read_system_file "/sys/class/dmi/id/bios_vendor")"
    firmware_version="$(read_system_file "/sys/class/dmi/id/bios_version")"
    firmware_release="$(read_system_file "/sys/class/dmi/id/bios_release")"

    full_firmware="$firmware_type version $firmware_version, revision $firmware_release"

    board_name="$(read_system_file "/sys/class/dmi/id/board_name")"
    board_vendor="$(read_system_file "/sys/class/dmi/id/board_vendor")"
    ...
    # $current_date 
    echo ""
    # Use enhanced network status function
    ...
    # UPDATE
    # Initialize DAYS if not set (from calculate_time_since_last_update)
    if [[ -z "${DAYS:-}" ]]; then
        # Try to calculate if LAST_UPDATE_DATE is available
        if [[ -n "${LAST_UPDATE_DATE:-}" ]] && [[ "$LAST_UPDATE_DATE" != "Unavailable" ]]; then
            CURRENT_TIMESTAMP=$(date +%s)
...
# Complete System Information Using inxi
function complete_system_info {
    ...
    if command -v inxi &> /dev/null; then
        ...
        # Initialize VLEVEL if not set to avoid unbound variable error
        local vlevel_arg="${VLEVEL:-}"
        if [[ -n "$vlevel_arg" ]]; then
            pprint "inxi -Fz --system --max-wrap 10 --indents 10 -c 37 --width -1 $vlevel_arg"
        else
            pprint "inxi -Fz --system --max-wrap 10 --indents 10 -c 37 --width -1"
        fi
    fi
}