...
1. Update /modules/info/boot to capture warning and errors
2. Update /modules/info/test to improve kernel/hardware/boot modules
3. Add /modules/info/security functions (recommends could live here?)
4. Grab the logic from /modules/base/netinfo to get the network status (if protected or exposed) so it can be added to the /modules/base/default & /modules/base/banner
5. Add the missing things from below
